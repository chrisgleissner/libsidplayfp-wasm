...
export function measure(samples) {
  ...
  return {
    ...
    dc,
    rms,
    // A held DC frame can have non-zero RMS. Playback health must require AC
    // energy as well, otherwise a tune that never advances can look audible.
    acRms: Math.sqrt(Math.max(0, rms * rms - dc * dc)),
    peak,
...
export function correlation(a, b) {
  ...
  // Constant streams have no defined Pearson correlation. When they are
  // sample-identical the native-parity question has a clear answer: match.
  if (denominator === 0) {
    for (let i = 0; i < n; i++) {
      if (a[i] !== b[i]) return 0;
    }
