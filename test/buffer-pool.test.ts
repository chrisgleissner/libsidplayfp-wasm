/** Lifecycle safety tests for context replacement and disposal. */
...
describe("SidAudioEngine lifecycle", () => {
  it("should not retain a module resolved after dispose while ensureModule is awaiting", async () => {
    ...
    await expect(pendingModule).rejects.toThrow(
      "SidAudioEngine has been disposed",
    );
  ...
  it("constructs and disposes without loading a module", async () => {
  ...
  it("releases cached playback state on dispose()", async () => {
    const engine = new SidAudioEngine({ sampleRate: 44100, stereo: true });
    await engine.loadSidBuffer(testSidData);

    // Render some audio before releasing the wrapper.
  ...
  it("replaces a context safely when loading another tune", async () => {
    const engine = new SidAudioEngine({ sampleRate: 44100, stereo: true });
  ...
  it("keeps multiple engine contexts independent", async () => {
