...
describe('Audio Continuity Verification', () => {
    ...
    test('serializes rendered audio as a valid in-memory WAV payload', async () => {
        const engine = new SidAudioEngine({ sampleRate: 44100, stereo: true });
        const sidBytes = loadRealSid();
        await engine.loadSidBuffer(sidBytes);
        ...
        expect(new TextDecoder().decode(wavBuffer.subarray(0, 4))).toBe('RIFF');
        expect(new TextDecoder().decode(wavBuffer.subarray(8, 12))).toBe('WAVE');
        expect(view.getUint32(40, true)).toBe(pcm.byteLength);
        expect(wavBuffer.byteLength).toBe(44 + pcm.byteLength);
    });
});
