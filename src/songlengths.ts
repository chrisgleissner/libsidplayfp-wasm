...
export class SonglengthDatabase {
    ...
    private constructor(entries: Map<string, readonly number[]>) {
    ...
    static parse(text: string): SonglengthDatabase {
        ...
        for (const rawLine of text.split(/\r?\n/)) {
            ...
            if (malformed || lengths.length === 0) continue;
     ...
     * straight through.
     */
    lengthMs(md5: string, song = 1): number | null {
        const lengths = this.#entries.get(md5.trim().toLowerCase());
        if (!lengths) return null;
        if (!Number.isInteger(song) || song < 1 || song > lengths.length) return null;
        return lengths[song - 1] ?? null;
    }

    /** As {@link lengthMs}, in seconds. */
    lengthSeconds(md5: string, song = 1): number | null {
 ...
 */
function parseTimeMs(field: string): number | null {
    ...
    if (match[3] === undefined && field.charAt(match[0].length) === ".") return null;

    const minutes = Number(match[1]);
    const seconds = Number(match[2]);
    let milliseconds = 0;
    if (match[3] !== undefined) {
        if (match[3].length > 3) return null;
